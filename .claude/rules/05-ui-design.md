...
- **Font Awesome 6.7.2** — có trong base.html nhưng floating controls đã bị xóa, không còn dùng cho theme toggle
...
- **Hàm JS**: `applyTheme(theme)` trong `static/js/main.js` — chỉ cập nhật `data-theme` và localStorage (không còn cập nhật icon/label vì floating controls đã xóa)
...
| `dep.*` | trang nạp tiền |
| `prof.*` | trang hồ sơ (bao gồm `prof.label.phone`, `prof.label.address`, `prof.label.email`) |
| `trad.*` | trang AI Trading (`trad.hero.*`, `trad.algo.*`, `trad.bt.*`, `trad.cta.*`) |
| `settings.*` | trang cài đặt (`/profile/settings/`) |
...
- **Scroll fade-in**: class `animate-on-scroll` (không phải `animate-fadeInUp`) kết hợp `IntersectionObserver`
  - CSS dùng `@keyframes scrollFadeUp` (không dùng `transition`) để animation có thể restart:
    ```css
    ...
    ```
  - JS: khi scroll vào → `classList.remove('visible')` + `void el.offsetWidth` (force reflow) + `classList.add('visible')`. Khi scroll ra → remove `visible`. Pattern này đảm bảo animation chạy lại mỗi lần.
  - **Exception**: `.error-page .animate-on-scroll { opacity: 1; transform: translateY(0); }` — trang 404 hiện ngay không cần scroll
- Loading state: skeleton screen (không dùng spinner đơn thuần)
- Số liệu tăng: count-up animation khi vào viewport
- `.btn-ghost-sm` (nút Đăng nhập): `border: 1px solid transparent`; hover → `border-color: var(--accent)` — viền xuất hiện khi hover
